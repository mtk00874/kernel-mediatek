...
{
    unsigned int cmp;
    struct clock_event_device *evt = &mt8127_gpt.clockevent;
    struct gpt_device *dev = id_to_dev(GPT_CLKEVT_ID);
...
{
    struct clocksource *cs = &mt8127_gpt.clocksource;
    struct gpt_device *dev = id_to_dev(GPT_CLKSRC_ID);
...
struct mt_clock *mt8127_clocks[] =
{
    &mt8127_gpt,
};

void __init mt8127_timer_init(void)
{
    int i;
    struct mt_clock *clock;
    int err;

    for (i = 0; i < ARRAY_SIZE(mt8127_clocks); i++) {
        clock = mt8127_clocks[i];
        ...
        if (err) {
            pr_err("mt8127_timer_init: setup_irq failed for %s\n", clock->irq.name);
        }
